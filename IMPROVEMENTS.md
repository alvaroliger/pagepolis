...
- ✅ **Auditar rendimiento del hero 3D en móviles de gama baja** — `devicePerfTier()` en
  `hero3d.js` y `Hero3D.tsx` mide `navigator.hardwareConcurrency`/`deviceMemory`: gama muy
  baja (≤2 núcleos o ≤2GB) desactiva el motor WebGL entero (se queda el mesh-gradient de
  `base.css`), gama media (≤4 núcleos o ≤4GB) baja a 3-4 figuras y `devicePixelRatio` a 1
  en vez de 1.75. Aplicado tanto a las webs generadas como a la propia app (landing,
  dashboard, wizard).
...
- Rendimiento del hero 3D en gama baja/media: `devicePerfTier()` desactiva el motor WebGL
  entero en gama muy baja y baja figuras/resolución en gama media, en `hero3d.js` (webs de
  clientes) y `Hero3D.tsx` (la propia app). Suite 244/244 verde, `npm run build` OK.

## Ideas nuevas
- 🟢 Tests para `pagepolis:weekly-reports` (mismo patrón; cero cobertura para el comando de informes semanales).
- 🟢 Arreglar mutación de Carbon en `SendWeeklyReports::buildStats()` (`$weekAgo->subDay()` muta el objeto, sesga la comparación semanal 8 días vs 7 días).
- 🔵 **Elección explícita "Simple | 3D" en el flujo de creación** — hoy `Templates/Index.tsx`
  solo filtra por `category`; no hay forma de que el cliente sepa de un vistazo qué
  plantillas llevan hero 3D antes de entrar al modal de preview. Añadir un dato `has3d` (o
  similar) a la plantilla, un filtro/toggle "Simple | 3D" en la galería y una insignia en la
  tarjeta — apoya directamente el pilar A de la misión (elegir 3D de forma clara en el
  wizard).
- 🟢 **Preview del hero 3D directamente en la tarjeta de la galería** (miniatura con canvas
  activo o vídeo/gif corto en loop al hacer hover), no solo dentro del modal — hoy hay que
  abrir el modal para ver el efecto 3D, lo que esconde el diferencial de las plantillas 3D.
- 🟢 Indicador en el editor (`Editor/Index.tsx`) de si la página actual usa el motor hero 3D
  (badge o toggle), para que el cliente sepa si puede aprovechar `tilt-3d`/`hero3d-canvas`
  al editar HTML/CSS a mano.
- 🟢 Revisar accesibilidad del hero 3D: el `<canvas>` ya lleva `aria-hidden`, pero falta
  comprobar que el CTA/texto del hero mantiene contraste AA sobre las figuras 3D en todas
  las paletas de marca (`--brand`) generadas, no solo la violeta por defecto.
