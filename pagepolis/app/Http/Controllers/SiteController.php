...
BADGE;
        ...
        $leadCapture = <<<JS
<script>(function(){var EP="{$leadEndpoint}";document.addEventListener("submit",function(e){var f=e.target;if(!f||f.tagName!=="FORM")return;e.preventDefault();e.stopImmediatePropagation();var d={},has=false;for(var i=0;i<f.elements.length;i++){var el=f.elements[i];if(el.name&&el.type!=="submit"&&el.type!=="button"){d[el.name]=el.value;if(el.value&&String(el.value).trim())has=true;}}if(!has){var m=f.querySelector("[data-pp-lead-msg]");if(!m){m=document.createElement("div");m.setAttribute("data-pp-lead-msg","");m.style.cssText="margin-top:10px;padding:10px 14px;border-radius:10px;background:#fef2f2;color:#991b1b;font:600 13px/1.4 system-ui,-apple-system,sans-serif;text-align:center";f.appendChild(m);}m.textContent="Completa al menos un campo antes de enviar.";return;}var b=f.querySelector("[type=submit],button");if(b){b.disabled=true;}fetch(EP,{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify({fields:d})}).then(function(r){return r.json().catch(function(){return{};});}).then(function(){f.innerHTML='<div style="padding:18px;border-radius:12px;background:#ecfdf5;color:#065f46;font:600 15px/1.45 system-ui,-apple-system,sans-serif;text-align:center">✅ ¡Gracias! Hemos recibido tu mensaje y te responderemos pronto.</div>';}).catch(function(){if(b){b.disabled=false;}alert("No se pudo enviar el mensaje. Inténtalo de nuevo.");});},true);})();</script>
JS;
